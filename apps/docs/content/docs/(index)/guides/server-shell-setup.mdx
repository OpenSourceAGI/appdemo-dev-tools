...
```bash
wget -qO- tinyurl.com/setupdev | bash
```

**Install all automated:**
```bash
 wget -qO- https://tinyurl.com/setupdev | bash -s -- all
```
**Install specific apps only:**
```bash
 wget -qO- https://tinyurl.com/setupdev | bash -s -- starship,docker,node
```

**Install Node with Volta** (no sudo permission issues)

```
wget -qO- https://tinyurl.com/nodeinstaller | bash
```
